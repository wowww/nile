...
export const MyPageNftListed = ({ tabKey, currentTabKey }: MyPageTabPros) => {
  const { t } = useTranslation(['mypage', 'common']);
  const api = NileApiService();
  const selectedWallet = useAtomValue(selectedWalletAtom);

  const [tokenList, setTokenList] = useState<NileNftToken[]>([]);
  ...
  const fetchTokens = useCallback(
    (page: number) => {
      api.mypage.nft
        .getOrderList(selectedWallet ?? '', page - 1, 12)
        .then(({ data }) => {
          // setNftList(data);
