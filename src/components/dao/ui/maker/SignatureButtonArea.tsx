  ...
  const ButtonList: ButtonListItemType[][] = [
    ...
    [
      ...
      },
      {
        type: 'outline',
        text: '임시 저장',
        onClick: () => message.info({ content: '저장 완료되었습니다.', key: 'temporary-store' }),
      },
      {
        type: 'bg',
        text: '서명 절차 진행',
    ...
    <div className={cn('signature-button-area')}>
      ...
      {ButtonList[step - 1].length > 0 && (
        <div className={cn('button-area')}>
          {ButtonList[step - 1].map((el, i) => {
            if (el.type === 'outline') {
              return <OutlineButton size="md" buttonText={el.text} color="black" onClick={el.onClick} key={el.text + i} />;
            } else if (el.type === 'bg') {
              return <BgButton size="md" buttonText={el.text} color="black" disabled={el.disabled} key={el.text + i} />;
            }
          })}
        </div>
      )}
