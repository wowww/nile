    ...
    <div className={cn('comment-view-wrap', [type])}>
      {comment.link ? (
        <>
          {type === 'governanceList' && (
            <>
              {/* governance/check 페이지 상단 헤더에서 커멘트 버튼을 클릭하면 스크롤 이동 합니다 */}
          ...
          {/* governance에서 governance 각 card에 존재하는 커멘트 버튼을 클릭하면 check 페이지로 이동하고 
          GovernanceCheckCommentsView:112에 있는 open을 활성화 시켜야 합니다. */}
          {type !== 'governanceList' && (
            <>
      ...
      <span className={cn('watch')}>
        <ReactSVG src="https://nile.blob.core.windows.net/images/assets/images/icon/ico_dao_watch.svg" />
