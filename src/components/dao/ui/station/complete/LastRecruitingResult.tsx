...
const LastRecruitingResult = () => {
  ...
  const isBtn = Dummy.length > 1 ? true : false;
  // const handleOpen = () => {
  //   setIsOpen(!isOpen);
  // };
  ...
  return (
    <div className={cn('last-recruiting-result-wrap', isBtn && 'has-btn')}>
      ...
      <ResultItem data={Dummy} isOpen={isOpen} isBtn={isBtn} setIsOpen={setIsOpen} />
    </div>
  );
};
...
const ResultItem = ({ data, isOpen, isBtn, setIsOpen }: resultItemType) => {
  const offset = useAtomValue(windowResizeAtom);
  const listRef = useRef<HTMLUListElement>(null);
  const lottieIndex = data.findIndex((el) => el.isSuccess === true);
  const { t } = useTranslation('dao');
  const resizeEvtInit = useWindowResize();
  useEffect(() => {
    document.fonts.ready.then(() => {
      if (listRef.current && !isOpen && data.length > 1) {
