        ...
        <div className={cn('info-area')}>
          <div className={cn('left')}>
            <div className={cn('main-info')}>
              <div className={cn('figure', areaType)}>
                <div className={cn('amount-data')}>
                  ...
                  {!isMobile && areaType !== 'trust' && areaType !== 'home' && (
                    <div className={cn('date')}>{areaType === 'treasury' ? date : dayjs.utc(selectedItem.date).local().format('YYYY-MM-DD mm:ss')}</div>
                  )}
                </div>
                {areaType === 'treasury' && (
                  ...
                  <div className={cn('flow-info')}>
                    ...
                    <div className={cn('info-row')}>
                      <span className={cn('info-name')}>
                        ...
                        {t('incinerator.chart.legend.0', { type: useDaoCharacterConvert(activeDao.value) })} : $
                      </span>
                ...
                {areaType !== 'treasury' && areaType !== 'incinerator' && areaType !== 'trust' && areaType !== 'home' && (
                  <span className={cn('dollar-tag')}>$1,200,000.00</span>
                )}
                {/* 23.03.22 수정: 조건 추가 */}
              ...
              {areaType === 'trust' && <div className={cn('date')}>{moment(selectedItem.date).format('YYYY-MM-DD mm:ss')}</div>}
              {areaType === 'home' && <div className={cn('date')}>{moment(selectedItem.date).format('YYYY-MM-DD mm:ss')}</div>}
              {isMobile && areaType !== 'trust' && areaType !== 'home' && (
                <div className={cn('date')}>{areaType === 'treasury' ? date : moment(selectedItem.date).format('YYYY-MM-DD mm:ss')}</div>
              )}
            </div>
          </div>
